...
#negative log-likelihood of cumulative Gaussian fit, takes parameter vector and data as input
nll.bimodal <- function(p, d) { 
  #read out expected cumulative gaussian probabilities for each distance
  phi <- (p[1] + (1-2*p[1])*pnorm(d$SOA, (d$rightModality == 'visual') * p[2] + (d$rightModality != 'visual') * p[4], p[3]))
  #ensure not to hit boundaries, otherwise log will cause problems
  phi <- pmax(pmin(phi, .99999), .00001)
  #negative log-likelihood of probability to get the number of right responses in the number of trials given probability phi
  -sum(d$noFurtherRight * log(phi) + (d$noTrials - d$noFurtherRight) * log(1 - phi) )
...
library(phia)
#faster computations with larger datasets
library(data.table)
...
#store colours
myColours.VA <- c('#7897AA','#e0e1e0', '#104c74')
...
setwd(paste(path,"/data/expIndia_AV/prepared",sep=""))
...
#list all txt files in data directory
listOfFiles = list.files(pattern= glob2rx("TOJ_*_VA.txt"))
#read every .txt file in and append it to data frame
dataSet.even.newer  <-  do.call(rbind, lapply(listOfFiles, function(fname) {
  #print to console the name of the file
  print(fname)
  #read in the file
  dum  <-  read.table(fname, header = TRUE, sep="\t")
...
subjectList.India <- read.table("TOJ_VA.txt", header = TRUE, sep="\t", as.is = T)

#change working directory to data directory
setwd(paste(path,"/data/expIndia_AV/prepared_HH",sep=""))
#list all txt files in data directory
listOfFiles.HH = list.files(pattern= "_VA.txt")
#read every .txt file in and append it to data frame
...
#read in subject list
subjectList.HH <- read.table("TOJ_VA_HH.txt", header = TRUE, sep="\t", as.is = T)

#change working directory to data directory
setwd(paste(path,"/data/expIndia_AV/prepared_HH_control",sep=""))
#list all txt files in data directory
listOfFiles.HH.control = list.files(pattern= "_VA.txt")
#read every .txt file in and append it to data frame
...
#read in subject list
subjectList.HH.control <- read.table("TOJ_VA_HH_control.txt", header = TRUE, sep="\t", as.is = T)

#change working directory to data directory
setwd(paste(path,"/data/expIndia_AV/prepared_control",sep=""))
#list all txt files in data directory
listOfFiles.control = list.files(pattern= "_VA.txt")
#read every .txt file in and append it to data frame
...
#recode modality
dataSet.old$firstModality <- factor(dataSet.old$firstModality, levels=c('A', 'T', 'V'), labels = c("auditory", "tactile", "visual"))
#recode modality
dataSet.old$secondModality <- factor(dataSet.old$secondModality, levels=c('A', 'T', 'V'), labels = c("auditory", "tactile", "visual"))

#recode modality
dataSet.newer$firstModality <- factor(dataSet.newer$firstModality, 
                                      levels = c("audioFirst", "tactileFirst", "visualFirst"), 
                                      labels = c("auditory", "tactile", "visual"))
#recode modality
dataSet.newer$secondModality <- factor(dataSet.newer$secondModality, 
                                       levels = c("audioSecond", "tactileSecond", "visualSecond"), 
                                       labels = c("auditory", "tactile", "visual"))
#recode modality
dataSet.even.newer$firstModality <- factor(dataSet.even.newer$firstModality, 
                                           levels = c("audioFirst", "tactileFirst", "visualFirst"), 
                                           labels = c("auditory", "tactile", "visual"))
#recode modality
dataSet.even.newer$secondModality <- factor(dataSet.even.newer$secondModality, 
                                            levels = c("audioSecond", "tactileSecond", "visualSecond"), 
                                            labels = c("auditory", "tactile", "visual"))
#recode modality
...
dataSet.newer$firstSide <- factor(dataSet.newer$firstSide, 
                                  ...
                                  labels = c("left", "right"))
#recode modality
dataSet.even.newer$firstSide <- factor(dataSet.even.newer$firstSide, 
                                       levels = c("leftSideFirst", "rightSideFirst"), 
                                       labels = c("left", "right"))
#recode modality
...
#missing columns will be filled with NA
dataSet <- rbind.fill(dataSet, dataSet.even.newer)
#missing columns will be filled with NA
...
#add subject list
dataSet <- merge(dataSet, rbind.fill(subjectList.India, subjectList.HH, subjectList.HH.control, subjectList.control), by="subNo")

#remove single datasets
rm(dataSet.old, dataSet.newer, dataSet.even.newer, dataSet.newer.control, dataSet.newer.HH, dataSet.newer.HH.control)
...
dataSet[, modalityMatch := ifelse(modalityCombination == 'VV' | modalityCombination == 'AA', 'unimodal', 'bimodal')]
...
dataSet[, modalityCondition := ifelse(modalityMatch == 'unimodal', 
                                      ifelse(firstModality == 'auditory', 
                                             'auditory', 
                                             'visual'),
                                      'bimodal')]
#set order of levels
dataSet$modalityCondition <- factor(dataSet$modalityCondition, levels = c('bimodal', 'visual', 'auditory'))
...
clean <- function(x) {replace(x, x > mean(x, na.rm=T) + 2.5 * sd(x, na.rm=T)  | x < 200  | x > 40000, NA)}
#clean the data for each subject and condition (reorders the data by conditions as a side product)
dataSet[, RTclean := clean(RT), by = subNo]
...
write.table(subset(dataSet, action == 'keep'),
            file = paste(path, '/data/expIndia_AV/aggregated/expXV_India_AV_dataSet.txt', sep = ''),
...
meanValuesSOAGroup <- meanValuesSOASubj[action == 'keep', list(propFurtherRight.M = mean(propFurtherRight),
                                  ...
                                  by = c('SOA', 'modalityMatch', 'rightModality', "subversion", "group", 'action')]

#function to wrap call to fitting function
fitCumulativeGaussian <- function(x){
  #fit sigmoid
  parameters <- optim(c(0.3, 0, 20), nll, d = x, lower = c(0.0, -400, 5), upper = c(0.3, 400, 2000), method='L-BFGS-B')$par
  #return parameters as list
  list(sigma=parameters[3], lambda=parameters[1], mu=parameters[2])
}

#function to wrap call to fitting function
fitCumulativeGaussian.bimodal <- function(x){
  #fit sigmoid
  parameters <- optim(c(0.3, 0, 20, 0), nll.bimodal, d = x, lower = c(0.0, -400, 5, -400), upper = c(0.3, 400, 2000, 400), method='L-BFGS-B')$par
  #return parameters as list
  list(sigma=parameters[3], lambda=parameters[1], mu=parameters[2], mu2=parameters[4])
}
...
meanValuesSOAGroup[,  c('sigma', 'lambda', 'mu') := fitCumulativeGaussian(list(SOA=SOA,noTrials=noTrials, noFurtherRight=noFurtherRight)),
                   by = c('modalityMatch', 'rightModality', "subversion", "group", 'action')]
...
curvesSOAGroup <- merge(unique(meanValuesSOAGroup[,c("group", "group2", 'modalityMatch', "rightModality", 'action', 'sigma', 'lambda', 'mu')]),
...
# store mean values in a txt file
write.table(subset(meanValuesSubj, action == 'keep'),
  file = paste(path, '/data/expIndia_AV/aggregated/expXV_India_AV_meanValues.txt', sep = ''),
  quote = F, sep = '\t', row.names = F, col.names = T)
...
gp <- ggplot(data = meanValuesSOAGroup, aes(x=SOA, y=propFurtherRight.M, colour=rightModality))
#set theme
gp <- gp + theme_bw(8)
#plot mean values per condition as point
gp <- gp + geom_errorbar(aes(ymin=propFurtherRight.M-propFurtherRight.SE, ymax=propFurtherRight.M+propFurtherRight.SE), width=50, size = 0.2)
#plot mean values per condition as line
gp <- gp + geom_line(data = curvesSOAGroup, size = 0.3)
#plot mean values per condition as point
...
#set colours
gp <- gp + scale_colour_manual(values=myColours.VA[c(3,2)], name = "Modality at right side")
#legend
...
gp
#set filename
filename="/results/expIndia_AV/expIndia_AV_TOJ_PsychPhysFunct_modalityMatch_rightModality_Fig_1.pdf"
#save picture
ggsave(paste(path,filename,sep=""), width = 9, height = 6, unit="cm")


#set basic layout of graph
gp <- ggplot(data = subset(meanValuesGroup, modalityCondition == 'bimodal'),
...
#plot mean values per condition as point
gp <- gp + geom_bar(stat="identity", position="dodge", colour = '#7897AA', fill = '#7897AA')
#set y-axis label
...
gp
#set filename
filename <- "/results/expIndia_AV/expIndia_AV_TOJ_visualFirst_Fig_2.pdf"
#save picture
ggsave(paste(path,filename,sep=""), width = 5.25, height = 5, unit="cm")

#set basic layout of graph
gp <- ggplot(data = subset(meanValuesGroup),
...
gp <- gp + scale_fill_manual(values=myColours.VA, name = "Modality Condition")
gp <- gp + scale_colour_manual(values=myColours.VA, name = "Modality Condition")
#set y-axis label
...
gp
#set filename
filename <- "/results/expIndia_AV/expIndia_AV_TOJ_accuracy_Fig_3.pdf"
#save picture
ggsave(paste(path,filename,sep=""), width = 5.25, height = 5, unit="cm")
...
#set basic layout of graph
gp <- ggplot(data = subset(meanValuesSOASubj, subNo == subjList[ii]), 
             aes(x=SOA, y=propFurtherRight, colour=rightModality))
#plot mean values per condition as line
gp <- gp + geom_line(data = subset(curvesSOAGroup, group == meanValuesSOASubj[subNo == subjList[ii], group]), 
...
gp
#set filename
filename=paste("/results/expIndia_AV/singleSubjects/expIndia_AV_TOJ_PsychPhysFunct_", subjList[ii], ".pdf", sep='')
#save picture
ggsave(paste(path,filename,sep=""), width = 6.0, height = 5, unit="cm")

#set basic layout of graph
gp <- ggplot(data = subset(meanValuesSubj, modalityCondition == 'bimodal' & subNo == subjList[ii]),
...
gp
#set filename
filename <- paste("/results/expIndia_AV/singleSubjects/expIndia_AV_TOJ_visualFirst_", subjList[ii], ".pdf", sep='')
#save picture
ggsave(paste(path,filename,sep=""), width = 4.2, height = 3.8, unit="cm")

#set basic layout of graph
gp <- ggplot(data = subset(meanValuesSubj, subNo == subjList[ii]),
             aes(x=modalityCondition, y=accuracy, group = modalityCondition, fill = modalityCondition, colour = modalityCondition))
#set theme
gp <- gp + theme_bw(8)
# #split by modality condition
# gp <- gp + facet_grid(modalityCondition ~.)
...
gp
#set filename
filename <- paste("/results/expIndia_AV/singleSubjects/expIndia_AV_TOJ_accuracy", subjList[ii], ".pdf", sep='')
#save picture
ggsave(paste(path,filename,sep=""), width = 4.2, height = 3.8, unit="cm")
...
visualFirst.GLMM <- glmer(visualFirstResp ~ group + (1|subNo),
                          dataSet[modalityMatch == 'bimodal' & action == 'keep' & !is.na(visualFirstResp) & abs(SOA) < 500 ,],
...
accuracy.GLMM <- glmer(accuracy ~ group * modalityCondition + (1|subNo),
                       dataSet[action == 'keep' & !is.na(accuracy) & abs(SOA) < 500 ,],
                       family="binomial", control=glmerControl(optimizer="bobyqa"))
                                                      ...
                                                      ifelse(pValue < 0.01, '**', 
                                                             ifelse(pValue < 0.05, '*', 
                                                                    ifelse(pValue < 0.1, '.',
                                                                           ' ')))))]
#loose unintended comparisons and select variables
print(accuracy.GLMM.interactions.II[is.element(groupDifference, c("CC-MCC", "DC-MDC")), 
